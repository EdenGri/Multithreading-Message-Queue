...
public class ExampleMessageSenderService extends MicroService {
    ...
    protected void initialize() {
        ...
        if (broadcast) {
            sendBroadcast(new BroadcastExmpl(getName()));
            System.out.println("Sender " + getName() + " publish an event and terminate");
            terminate();
        } else {
            Future<String> futureObject = (Future<String>)sendEvent(new EventExmpl(getName()));
            if (futureObject != null) {
            	...
            	System.out.println("No Micro-Service has registered to handle EventExmpl events! The event cannot be processed");
            }
            terminate();
        }
    }
